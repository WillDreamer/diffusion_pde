...
from datasets import PDEBench, PDEBench_npy
...
def main(args):
    ...
    if rank == 0:
        ...
        tb_dir = f"/panjiashu/EXPERIMENTS/Latte/log_tb/{args.exp_name}"  # The original tensorboard log path doesn't work for vscode now, idk why
        os.makedirs(experiment_dir, exist_ok=True)
        os.makedirs(tb_dir, exist_ok=True)
        logger = create_logger(experiment_dir)
        tb_writer = create_tensorboard(tb_dir)
        OmegaConf.save(args, os.path.join(experiment_dir, 'config.yaml'))
        os.system(f"cp -r /panjiashu/foundation_pde {experiment_dir}")
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, video_data in enumerate(tr_loader):
            ...
            x = video_data['video'].to(device, non_blocking=True) # [b f c h w]
            grid = video_data['grid'].to(device, non_blocking=True) if args.use_coordinates else None # [b 2 h w]
            equation_name = video_data['equation_name']
            # with torch.no_grad():
            ...
            if args.extras == 78: # text-to-video
                ...
                model_kwargs = dict(y=equation_name)
            else:
                model_kwargs = dict(y=None)
            if args.num_initial_steps > 0 and "STDiT" in args.model:
                model_kwargs["initial_steps"], x = x.split([args.num_initial_steps, x.shape[1] - args.num_initial_steps], dim=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="/wanghaixin/diffusion_pde/configs/pdebench/pdebench_train.yaml")
