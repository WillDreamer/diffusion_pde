...
class STDiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, d_s=256, d_t=16, **block_kwargs):
        ...
        self.attn_s = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        self.attn_t = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
...
class STDiT(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.learn_sigma = learn_sigma
        self.in_channels = in_channels + 2 if use_coordinates else in_channels
        self.out_channels = in_channels * 2 if learn_sigma else in_channels
        self.patch_size = patch_size
        self.num_heads = num_heads
        self.extras = extras
        ...
        self.t_embedder = TimestepEmbedder(hidden_size)
        if initial_dropout_prob > 0:
            self.dropper = condition_dropper(hidden_size=hidden_size, dropout_prob=initial_dropout_prob)
    ...
    def initialize_weights(self):
        ...
        if self.initial_dropout_prob > 0:
            nn.init.normal_(self.dropper.mask_token, std=0.02)
    ...
    def forward(self, 
        ...
        if use_fp16:
            x = x.to(dtype=torch.float16)
            initial_steps = initial_steps.to(dtype=torch.float16)
        ...
        if self.num_initial_steps == 1:
            ...
            if self.use_coordinates:
                x = self.x_embedder(torch.cat((x, grid.repeat(self.num_frames, 1, 1, 1)), dim=1))
            else:
                x = self.x_embedder(x) + self.pos_embed
            x = rearrange(x, '(b t) s c -> b (t s) c', b=batches)
            initial_steps = rearrange(initial_steps, 'b f c h w -> (b f) c h w')
            if self.use_coordinates:
                initial_steps = self.x_embedder(torch.cat((initial_steps, grid.repeat(self.num_initial_steps, 1, 1, 1)), dim=1))
            else:
                initial_steps = self.x_embedder(initial_steps) + self.pos_embed
            initial_steps = rearrange(initial_steps, '(b f) s c -> b (f s) c', b=batches)  
            if self.initial_dropout_prob > 0:
                initial_steps = self.dropper(initial_steps, training=self.training, use_cfg=use_cfg)
        
        if self.num_initial_steps > 1:
            # inconsistent with previous implementation due to the use of temporal embedding
            # b t c h w -> b (t s) d
            x = torch.cat((initial_steps, x), dim=1)
            x = rearrange(x, 'b f c h w -> (b f) c h w')
            ...
            x += self.temp_embed
            initial_steps, x = x[:, :self.num_initial_steps], x[:, self.num_initial_steps:]
            x = rearrange(x, '(b s) f c -> b (f s) c', b=batches)
            initial_steps = rearrange(initial_steps, '(b s) f c -> b (f s) c', b=batches)
            if self.initial_dropout_prob > 0:
                initial_steps = self.dropper(initial_steps, training=self.training, use_cfg=use_cfg)       

        for i, block in enumerate(self.blocks):
            x = block(x, t, initial_steps, self.temp_embed)
...
def STDiT_S_2(**kwargs):
    return STDiT(depth=8, hidden_size=384, patch_size=2, num_heads=6, **kwargs)
